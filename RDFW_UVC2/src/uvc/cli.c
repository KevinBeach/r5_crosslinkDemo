...
void process_command(const char *cmd)
{
    ...
    if (cmd[0] == 'V') {
        ...
        if (sscanf(cmd, "V %3s", tmp_arg) == 1) {
            ...
            if (cli_verbose)
                printf("\nERR: V command requires ON or OFF\r\n");
        }
        return;
    }
    ...
    if (type == 'S') {
        ...
        if (sscanf(cmd, "%*c %*c %4s", tmp) != 1) {
            if (cli_verbose)
                printf("\nERR: Cannot parse sensor address\r\n");
            return;
        }
    ...
    /* ---------------- CCM commands (C) ---------------- */
        ...
        if (rw == 'W') {
            ...
            if (coeff_int < -99 || coeff_int > 99) {
                if (cli_verbose)
                    printf("\nERR: CCM coefficient out of range (-99..99)\r\n");
                return;
            }
    ...
    /* ---------------- T commands (translate matrix) ---------------- */
        ...
        if (rw == 'W') {
            ...
            if (data_int < -255 || data_int > 255) {
                if (cli_verbose)
                    printf("\nERR: T data out of range (-255..255)\r\n");
                return;
            }
