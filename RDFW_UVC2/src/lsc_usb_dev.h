...
extern uint8_t print_buf[256], res_buf[256] __attribute__ ((aligned(8)))/*__attribute__((section(".myBufSection")))*/;

#define LSC_USB_IN_EP       1  /**< USB IN endpoint number. */
#define LSC_USB_OUT_EP      1  /**< USB OUT endpoint number. */

#define LSC_IEBM  /**< Enable LSC_IEBM to enable IEBM flow. */
...
/* Enable LSC_LPM_ENABLE to enable LPM in USB 2.0*/
#define LSC_LPM_ENABLE

/* Enable LSC_U1_U2 for USB 3.0*/
#define LSC_U1_U2
...
/* Enable LINK_TRB_USE to enable link trb flow use for transfer */
/* Disable LINK_TRB_USE to enable single trb flow use for transfer */
#define LINK_TRB_USE

// Define capacity of allocating max buf size per trb
#define MAX_TRB_SIZE    LSC_IEBM_FW_SINGLE_BUF_DEPTH//16384

// Define capacity of queue(struct lsc_ep_buf_info ep_buf_i[NO_OF_BUF]) for storing buffer details
#define NO_OF_BUF 10
...
/**
 * @brief Macro to get command from DEP command.
 */
#define LSC_DEPCMD_CMD(x) ((x) & 0xf)
...
#define NO_OF_TRB_PER_EP        8   /**< number of TRB*/
...
#define LSC_ENDPOINTS_NUM       9	//18  /**< Total number of endpoint */
...
#define LSC_EVENT_BUFFERS_SIZE  1024 /**< event size * maximum number of event */
 ...
 * Refer sec 4.2.1 and 1.3.7 of Synopsis Programming Guide Version 3.30b
 */
...
} setup_pkt; /**< USB Standard Control Request */

/**
 * struct lsc_ep_buf_info - lsc_ep_buf_info Parameters
 * @param buf_ptr: Pointer to the buffer
 * @param no_of_event: Information number of event pending of current dequeued buffer
 * @param buf_size: Size of the buffer in bytes
 */
struct lsc_ep_buf_info {
    uint8_t *buf_ptr; /**< Pointer to the buffer */
    uint32_t no_of_event; /**< Information number of event pending of current dequeued buffer */
    uint32_t buf_size; /**< Size of the buffer in bytes */
};


// Forward declaration of struct lsc_usb_dev is used to remove warnings.
struct lsc_usb_dev;
 ...
 */
struct lsc_ep {
    void (*handler) (struct lsc_usb_dev*, uint8_t, uint32_t);
    /**< User handler called
...
//    struct lsc_trb ep_trb[NO_OF_TRB_PER_EP + 1] __attribute__ ((aligned(32)));
    ...
    uint32_t remain_bytes; /**< RemainBytes for transfer */
    uint8_t maxburst; /**< MaxBurst */

    struct lsc_ep_buf_info ep_buf_i[NO_OF_BUF]; /**< Circular queue to store buffer details */
    uint32_t ep_buf_i_rd; /**< Index of the next element to be read */
    uint32_t ep_buf_i_enqueue; /**< Index where the next element will be enqueued */
    uint32_t ep_buf_i_dequeue; /**< This is used for dequeuing the buffer */
};
...
//    uint8_t event_buf[LSC_EVENT_BUFFERS_SIZE] __attribute__((aligned(LSC_EVENT_BUFFERS_SIZE))); /**< Event buffer array */
    ...
    volatile uint8_t is_enum_done;/**< Flag - Check if enumeration is done or not */
...
//  uint8_t HasHibernation;                                 /**< Has hibernation support */
    ...
    void (*usb_class_req_handler) (struct lsc_usb_dev*, setup_pkt*);
    void (*usb_vendor_req_handler) (struct lsc_usb_dev*, setup_pkt*);
    void (*usb_class_spec_desc_req_handler) (void*);
    void (*lsc_usr_app_handler) (struct lsc_usb_dev*, uint8_t, uint8_t);
    struct timing u30_timing;
    uint8_t ep0_stall;
};

/**
 * USB Device Controller representation used for dedicated memory
 * for USB23 Controller
 */
/**
 * struct lsc_usb_dev - USB Device Controller representation
 * @param setup_data: Setup data packet
 * @param ep0_trb: TRB for control transfers
 * @param ep_trb: TRB for data transfers
 * @param event_buf: Event buffer array
 * @endcond
 */
struct lsc_usb_dev_dm {
    setup_pkt setup_data __attribute__ ((aligned(32))); /**< Setup Packet buffer */
    struct lsc_trb ep0_trb __attribute__ ((aligned(32))); /**< TRB for control transfers */
    struct lsc_trb ep_trb[9][NO_OF_TRB_PER_EP + 1] __attribute__ ((aligned(32)));	/**< TRB for data transfers */
    uint8_t event_buf[LSC_EVENT_BUFFERS_SIZE] __attribute__((aligned(LSC_EVENT_BUFFERS_SIZE))); /**< Event buffer array */
